 ...
 * @since 1.0
 */
class BiBao extends MY_Controller{
	/**
	 * 商户ID
	 * 
	 * @var string
	...
	public function __construct(){
		...
		$pay = $this->paymentmodel->getMerchant('bibao');
		if($pay){
	...
	/**
	 * 加密返回字符串
	 * @param  string $r0_Cmd   参照币宝支付协议
	 * @param  string $r1_Code  参照币宝支付协议
	 * @param  string $r2_TrxId 参照币宝支付协议
	 * @param  string $r3_Amt   参照币宝支付协议
	 * @param  string $r4_Cur   参照币宝支付协议
	 * @param  string $r5_Pid   参照币宝支付协议
	 * @param  string $r6_Order 参照币宝支付协议
	 * @param  string $r7_Uid   参照币宝支付协议
	 * @param  string $r8_MP    参照币宝支付协议
	 * @param  string $r9_BType 参照币宝支付协议
	 * @return string           参照币宝支付协议
	 */
	...
	/**
	 * 生成请求参数的加密结果
	 * @param  string $p0_Cmd          参照币宝支付协议
	 * @param  string $p2_Order        参照币宝支付协议
	 * @param  string $p3_Amt          参照币宝支付协议
	 * @param  string $p4_Cur          参照币宝支付协议
	 * @param  string $p5_Pid          参照币宝支付协议
	 * @param  string $p6_Pcat         参照币宝支付协议
	 * @param  string $p7_Pdesc        参照币宝支付协议
	 * @param  string $p8_Url          参照币宝支付协议
	 * @param  string $pa_MP           参照币宝支付协议
	 * @param  string $pd_FrpId        参照币宝支付协议
	 * @param  string $pr_NeedResponse 参照币宝支付协议
	 * @return string                  参照币宝支付协议
	 */
	...
	public function submit(){
		...
		$this->load->view('bibao_submit',$data);
	}
	...
	public function receive(){
		...
		$rp_PayDate=$_REQUEST['rp_PayDate'];//支付成功时间
		$ru_Trxtime=$_REQUEST['ru_Trxtime'];//时间戳 交易结果通知时间
		$hmac=$_REQUEST['hmac'];
		...
		if($bRet){
			if($r1_Code=="1"){
				...
				$r5_Pid = $r5_Pid;//用户字段
				$amount = $r3_Amt; 
				//$mydate = $rp_PayDate;//交易时间 此值为时间戳 - 2013.05.31 莫名其妙的不通知就变成了这样的格式2151645140130531，垃圾
				//$mydate = $ru_Trxtime;
				$mydate = strtotime($rp_PayDate);//支付成功时间。现在变成时间戳了 囧
				...
				if($my_config['is_rj']==1){
					...
					file_put_contents('./orderLog.log',date('Y-m-d H:i:s') . "\r\n" . json_encode($params) . 
						"\r\n" . json_encode($data) . "\r\n". json_encode($result) . "\r\n",FILE_APPEND);
					//备注：某些情况下$data为空数组操作也是成功的
