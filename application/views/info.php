...
<script type="text/javascript">
        function showTime(){
        	setTimeout(function() {
                ...
                $return_url = $this->session->flashdata('return_url') === false ?
                    $_SERVER['HTTP_REFERER'] : $this->session->flashdata('return_url');
                echo isset($_SERVER['HTTP_REFERER']) && $this->session->flashdata('info') != '注册成功！' 
                ? '"' . $return_url . '"' : '"' . site_url('user/login/' . urlencode($this->session->flashdata('regname'))) . '"';
                ?>;
            }, 3000);
